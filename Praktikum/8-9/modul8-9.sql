...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 28 Jun 2024 pada 21.50
...
(18, 15, 'Konsultasi', NULL, NULL, NULL, NULL),
(19, 15, 'Konsultasi', NULL, NULL, NULL, NULL);

--
-- Trigger `appointment`
--
DELIMITER $$
CREATE TRIGGER `check_bpjs_duplicate` BEFORE INSERT ON `appointment` FOR EACH ROW BEGIN
    DECLARE bpjs_count INT;

    -- Menghitung jumlah nomor BPJS yang sudah ada di tabel pasien
    SELECT COUNT(*)
    INTO bpjs_count
    FROM pasien
    WHERE no_bpjs_pasien = (SELECT no_bpjs_pasien FROM pasien WHERE id_pasien = NEW.id_pasien);

    -- Jika nomor BPJS sudah ada, atur tanggal dan jam appointment menjadi NULL
    IF bpjs_count > 0 THEN
        SET NEW.tanggal_mulai_appointment = NULL;
        SET NEW.tanggal_selesai_appointment = NULL;
        SET NEW.jam_mulai_appointment = NULL;
        SET NEW.jam_selesai_appointment = NULL;
    END IF;
END
$$
DELIMITER ;
...
--
-- Stand-in struktur untuk tampilan `appointment_valid`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `appointment_valid` (
`id_appointment` int(11)
,`nama_pasien` varchar(80)
,`jenis_kelamin` varchar(10)
,`jenis_appointment` varchar(40)
,`tanggal_mulai_appointment` date
,`jam_mulai_appointment` time
,`jam_selesai_appointment` time
);
...
--
-- Stand-in struktur untuk tampilan `frontdesk_view`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `frontdesk_view` (
`id_pasien` int(11)
,`nama_pasien` varchar(80)
,`jenis_kelamin` varchar(10)
,`jenis_appointment` varchar(40)
...
(15, 'John Doe', '1980-05-15', 'Laki-laki', 'Jl. Kenangan No. 123', '08123456789', 'john.doe@example.com', 110578432);

-- --------------------------------------------------------

--
-- Stand-in struktur untuk tampilan `recap_view`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `recap_view` (
`jenis_appointment` varchar(40)
,`jumlah_appointment` bigint(21)
,`nama_tenaga_medis` varchar(80)
,`jabatan` varchar(15)
,`spesialis_tenaga_medis` varchar(20)
);
...
--
-- Struktur untuk view `appointment_valid`
--
DROP TABLE IF EXISTS `appointment_valid`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `appointment_valid`  AS SELECT `a`.`id_appointment` AS `id_appointment`, `p`.`nama_pasien` AS `nama_pasien`, `p`.`jenis_kelamin` AS `jenis_kelamin`, `a`.`jenis_appointment` AS `jenis_appointment`, `a`.`tanggal_mulai_appointment` AS `tanggal_mulai_appointment`, `a`.`jam_mulai_appointment` AS `jam_mulai_appointment`, `a`.`jam_selesai_appointment` AS `jam_selesai_appointment` FROM (`appointment` `a` join `pasien` `p` on(`a`.`id_pasien` = `p`.`id_pasien`)) WHERE `a`.`tanggal_mulai_appointment` is not null AND `a`.`tanggal_selesai_appointment` is not null AND `a`.`jam_mulai_appointment` is not null AND `a`.`jam_selesai_appointment` is not null ;

-- --------------------------------------------------------

--
-- Struktur untuk view `frontdesk_view`
--
DROP TABLE IF EXISTS `frontdesk_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `frontdesk_view`  AS SELECT `p`.`id_pasien` AS `id_pasien`, `p`.`nama_pasien` AS `nama_pasien`, `p`.`jenis_kelamin` AS `jenis_kelamin`, `a`.`jenis_appointment` AS `jenis_appointment`, `a`.`tanggal_mulai_appointment` AS `tanggal_mulai_appointment`, `a`.`jam_mulai_appointment` AS `jam_mulai_appointment`, `a`.`jam_selesai_appointment` AS `jam_selesai_appointment` FROM (`pasien` `p` join `appointment` `a` on(`p`.`id_pasien` = `a`.`id_pasien`)) ;

-- --------------------------------------------------------

--
-- Struktur untuk view `recap_view`
--
DROP TABLE IF EXISTS `recap_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `recap_view`  AS SELECT `a`.`jenis_appointment` AS `jenis_appointment`, count(`a`.`jenis_appointment`) AS `jumlah_appointment`, `tm`.`nama_tenaga_medis` AS `nama_tenaga_medis`, `tm`.`jabatan` AS `jabatan`, `tm`.`spesialis_tenaga_medis` AS `spesialis_tenaga_medis` FROM ((`appointment` `a` join `detail_appointment` `da` on(`a`.`id_appointment` = `da`.`id_appointment`)) join `tenaga_medis` `tm` on(`da`.`id_tenaga_medis` = `tm`.`id_tenaga_medis`)) WHERE month(`a`.`tanggal_mulai_appointment`) = 4 GROUP BY `a`.`jenis_appointment`, `tm`.`nama_tenaga_medis`, `tm`.`jabatan`, `tm`.`spesialis_tenaga_medis` ;
...
--
ALTER TABLE `appointment`
  MODIFY `id_appointment` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;
